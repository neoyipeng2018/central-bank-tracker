#!/usr/bin/env python3
"""Generate a standalone HTML report of the BOE MPC Stance Tracker dashboard.

Usage:
    python generate_boe_html.py                 # writes boe_report_YYYY-MM-DD.html
    python generate_boe_html.py -o report.html  # custom output path
"""
...
SOURCE_LABELS = {
    ...
    "treasury_committee": "Treasury Cttee",
}
...
def build_dataframe(history):
    ...
    for p in PARTICIPANTS:
        ...
        rows.append(dict(
            name=p.name, short=last_name(p.name), role=p.role_type,
            title=p.title, internal=is_internal,
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        ...
        yaxis=dict(tickvals=list(range(len(df))),
                   ticktext=[f"{r.short}  ({r.role})" for _, r in df.iterrows()],
                   gridcolor=GRID),
...
def fig_scatter_2d(df):
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers+text",
        marker=dict(size=[18 if v else 12 for v in df["internal"]],
...
def fig_internals_vs_externals(df):
    """Internals vs Externals scatter. Returns (fig, ie_trace_names)."""
    idf = df[df.internal]
    edf = df[~df.internal]
    ia = idf["score"].mean()
    ea = edf["score"].mean()
    fig = go.Figure()
    ie_trace_names = []
    for group_df, label in [(idf, "Internals"), (edf, "Externals")]:
        ie_trace_names.append(list(group_df["name"]))
        fig.add_trace(go.Scatter(
    ...
    for avg, label in [(ia, "Internals"), (ea, "Externals")]:
        fig.add_trace(go.Scatter(
...
def fig_heatmap(history):
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        height=max(380, len(PARTICIPANTS) * 35),
...
def build_table_html(df):
    ...
    tbl = df[["name", "role", "title", "score", "label", "policy_score", "balance_sheet_score"]].copy()
    ...
    return f"""<table class="data-table">
        <thead><tr>
            <th>Name</th><th>Role</th><th>Title</th>
            <th>Score</th><th>Stance</th><th>Policy</th><th>BS Score</th>
        </tr></thead>
...
def generate_html(output_path: str):
    ...
    ie_fig, ie_trace_names = fig_internals_vs_externals(df)
    chart_ie = ie_fig.to_html(full_html=False, include_plotlyjs=False, div_id="ie-chart")
    ...
    # Internals/Externals: nested list of names per trace
    ie_trace_names_json = json.dumps(ie_trace_names)
...
<title>BOE MPC Stance Tracker — {date_str}</title>
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; }}
  ...
  #trend-detail, #spectrum-detail, #scatter-detail, #ie-detail, #heatmap-detail {{
    margin: 1rem 0 0.5rem 0;
    transition: opacity 0.2s;
  }}
  #trend-detail:empty, #spectrum-detail:empty, #scatter-detail:empty,
  #ie-detail:empty, #heatmap-detail:empty {{ display: none; }}
  .td-header {{
...
<!-- Hero -->
<div class="hero">
    <p class="hero-title">BOE MPC<br>Stance Tracker</p>
    <p class="hero-sub">Hawkish / dovish classification of Bank of England Monetary Policy Committee members
    based on recent news, speeches, MPC minutes, and public statements.</p>
...
<div class="metric-row">
    <div class="m-card m-hawk"><p class="m-label">Hawkish</p><p class="m-value">{len(hawks)}</p><p class="m-sub">{hawk_pct} of MPC</p></div>
    ...
    <div class="m-card m-dove"><p class="m-label">Dovish</p><p class="m-value">{len(doves)}</p><p class="m-sub">{dove_pct} of MPC</p></div>
    <div class="m-card m-bal"><p class="m-label">Hawk-Dove Balance</p><p class="m-value">{bal_str}</p><p class="m-sub">{bal_text}</p></div>
    <div class="m-card m-avg"><p class="m-label">MPC Average</p><p class="m-value">{avg_score:+.2f}</p><p class="m-sub">{score_label(avg_score).lower()} lean</p></div>
</div>

<!-- Chart 1: Spectrum -->
<hr class="divider">
<p class="section-hdr">Hawk-Dove Spectrum</p>
<p class="section-sub">All 9 MPC members ranked from most dovish to most hawkish &bull;
Larger dot = internal member &bull; Click any bar for details</p>
{chart_spectrum}
...
<p class="section-sub">Each member plotted by interest rate stance (x) and balance sheet stance (y) &bull; Dot size indicates internal/external status &bull; Click any point for details</p>
{chart_scatter}
<div id="scatter-detail"></div>

<!-- Composition + Internals/Externals (side by side) -->
<hr class="divider">
<div class="row-2col">
    <div>
        <p class="section-hdr">MPC Composition</p>
        <p class="section-sub">Stance breakdown across all 9 voting members</p>
        {chart_composition}
    </div>
    <div>
        <p class="section-hdr">Internals vs Externals</p>
...
<p class="section-sub">How each MPC member&rsquo;s stance has evolved over recent months &bull; Click any data point for details</p>
...
(function() {{
  ...
  var ieTraceNames = {ie_trace_names_json};
  ...
  /* Internals vs Externals chart */
  var ieEl = document.getElementById('ie-chart');
  if (ieEl) {{
    ieEl.on('plotly_click', function(data) {{
      if (!data || !data.points || !data.points.length) return;
      var pt = data.points[0];
      var curve = pt.curveNumber;
      var idx = pt.pointNumber;
      if (curve < ieTraceNames.length && idx < ieTraceNames[curve].length) {{
        showDetail('ie-detail', ieTraceNames[curve][idx], null, null);
      }}
    }});
  }}
...
<p class="section-sub">Monthly stance scores across all MPC members &bull; Click any cell for details</p>
{chart_heatmap}
<div id="heatmap-detail"></div>

<!-- Table -->
<hr class="divider">
<p class="section-hdr">MPC Member Details</p>
...
<p class="section-sub">News articles, speeches, MPC minutes, and quotes supporting each member&rsquo;s stance classification</p>
{evidence_html}
...
<div class="foot">
    ...
    Dual-dimension analysis: Policy (Bank Rate) + Balance Sheet (QT/QE/Gilt purchases)<br>
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate standalone BOE MPC Stance Tracker HTML report")
    parser.add_argument("-o", "--output", default=None, help="Output HTML file path")
    args = parser.parse_args()
    out = args.output or f"boe_report_{datetime.now():%Y-%m-%d}.html"
    generate_html(out)
