#!/usr/bin/env python3
"""BOE MPC Stance Tracker - Interactive Streamlit Dashboard."""
...
from boe_tracker.participants import PARTICIPANTS, get_internals, get_externals
...
from boe_tracker.policy_signal import (
    compute_weighted_signal,
    implied_rate_action,
    compute_meeting_drift,
    signal_vs_decisions,
)
from boe_tracker import boe_stats

# -- Page Config ------------------------------------------------------------
st.set_page_config(
    page_title="BOE MPC Stance Tracker",
    page_icon="https://upload.wikimedia.org/wikipedia/en/thumb/a/a9/Bank_of_England_logo.svg/200px-Bank_of_England_logo.svg.png",
    layout="wide",
    initial_sidebar_state="expanded",
)

# -- Custom CSS (same as FOMC dashboard) ------------------------------------
st.markdown(
    """
    <style>
    ...
    .hero-sub {
        font-size: 1.05rem; color: #64748b; margin: 0.6rem 0 0 0;
        font-weight: 400; line-height: 1.5;
    }
    .hero-date {
        display: inline-block; margin-top: 0.8rem; padding: 0.3rem 0.8rem;
        background: rgba(99, 102, 241, 0.12); border: 1px solid rgba(99, 102, 241, 0.25);
        border-radius: 20px; color: #818cf8; font-size: 0.78rem;
        font-weight: 600; letter-spacing: 0.03em; text-transform: uppercase;
    }

    .metric-row { display: flex; gap: 1rem; margin: 1.5rem 0 2rem 0; }
    .m-card {
        flex: 1;
        ...
        padding: 1.4rem 1rem; text-align: center; backdrop-filter: blur(10px);
        transition: border-color 0.2s;
    }
    .m-card:hover { border-color: rgba(148,163,184,0.2); }
    .m-label {
        font-size: 0.7rem; font-weight: 600; text-transform: uppercase;
        letter-spacing: 1.5px; color: #64748b; margin: 0 0 0.5rem 0;
    }
    ...
    .section-hdr {
        ...
        margin: 0 0 0.15rem 0; color: #f1f5f9;
    }
    .section-sub {
        font-size: 0.82rem; color: #64748b; margin: 0 0 1.2rem 0; font-weight: 400;
    }
    ...
    .sidebar-brand {
        ...
        color: #f1f5f9; margin: 0 0 0.3rem 0;
    }
    .sidebar-desc {
    ...
    .legend-item {
        ...
        margin: 0.35rem 0; font-size: 0.82rem; color: #94a3b8;
    }
    ...
    .ev-title {
        ...
        margin: 0 0 0.3rem 0; line-height: 1.4;
    }
    ...
    .ev-quote {
        ...
        margin: 0.4rem 0; padding-left: 0.8rem;
        border-left: 2px solid rgba(148,163,184,0.15); line-height: 1.5;
    }
    ...
    .signal-panel {
        ...
        padding: 2rem 2.2rem; margin: 0 0 1rem 0;
    }
    .signal-header {
    ...
    .signal-title-sub { font-size: 0.78rem; color: #64748b; margin: 0.2rem 0 0 0; }
    .signal-badge {
    ...
    .signal-cell-label {
        ...
        letter-spacing: 1.2px; color: #64748b; margin: 0 0 0.5rem 0;
    }
    .signal-cell-value {
        font-size: 1.8rem; font-weight: 800; margin: 0; line-height: 1.1;
    }
    .signal-cell-sub {
        font-size: 0.7rem; color: #475569; margin: 0.3rem 0 0 0; font-weight: 500;
    }
    .signal-meeting-row { display: flex; gap: 1rem; margin: 1rem 0 0 0; }
    .signal-meeting-card {
        flex: 1; background: rgba(15,23,42,0.4);
        border: 1px solid rgba(148,163,184,0.06); border-radius: 12px;
        padding: 1rem 1.2rem;
    }
    .signal-meeting-label {
        font-size: 0.65rem; font-weight: 600; text-transform: uppercase;
        letter-spacing: 1px; color: #64748b; margin: 0 0 0.4rem 0;
    }
    .signal-meeting-val {
        font-size: 0.95rem; font-weight: 600; color: #e2e8f0; margin: 0;
    }
    .signal-meeting-note {
        font-size: 0.72rem; color: #475569; margin: 0.25rem 0 0 0;
    }

    .foot {
        text-align: center; color: #475569; font-size: 0.75rem;
        padding: 1.5rem 0 0.5rem 0; line-height: 1.7;
    }
...
SOURCE_LABELS = {
    "duckduckgo": "News",
    "boe_speeches": "BOE Speech",
    "boe_news_rss": "BOE News",
    "mpc_minutes": "MPC Minutes",
    "treasury_committee": "Treasury Ctte",
}
...
def render_evidence_panel(name: str, history_data: dict, date: str | None = None):
    """Render evidence cards for a participant."""
...
# -- Sidebar ----------------------------------------------------------------
with st.sidebar:
    st.markdown('<p class="sidebar-brand">BOE MPC Stance Tracker</p>', unsafe_allow_html=True)
    st.markdown(
        '<p class="sidebar-desc">Hawkish / dovish classification of Bank of England '
        "Monetary Policy Committee members based on recent news, speeches, and public statements.</p>",
        unsafe_allow_html=True,
    )
    st.markdown("---")
    ...
    st.markdown("---")
    st.markdown("**Filters**")
    show_internals = st.checkbox("Internal members only", value=False)
    show_externals = st.checkbox("External members only", value=False)
    ...
    st.markdown(
        ...
        "matching on news headlines, BOE speeches, MPC minutes, and Treasury Committee hearings.</p>",
        unsafe_allow_html=True,
    )
...
for p in PARTICIPANTS:
    ...
    rows.append(
        dict(
            ...
            is_internal=p.role_type != "External Member",
        )
    )
...
filtered = df.copy()
if show_internals:
    filtered = filtered[filtered.is_internal]
if show_externals:
    filtered = filtered[~filtered.is_internal]
...
st.markdown(
    f"""<div class="hero">
        <p class="hero-title">BOE Monetary Policy<br>Committee Tracker</p>
        <p class="hero-sub">Real-time hawkish / dovish classification of Bank of England MPC members
...
st.markdown(
    f"""<div class="metric-row">
        ...
        <div class="m-card m-neut">
            ...
            <p class="m-sub">{len(neutrals)} centrist</p>
        </div>
...
if show_policy_signal:
    ...
    if _days_until is not None and _next_mtg:
        _mtg_date_str = _next_mtg.date.strftime("%B %d, %Y")
        if _days_until == 0:
    ...
    _rate_str = f"{_current_rate:.2f}%" if _current_rate is not None else ""

    _proj_str = ""
    if _action["projected_rate"] is not None:
        _proj_str = f"{_action['projected_rate']:.2f}%"
    ...
    _prev_date_str = _prev_mtg.date.strftime("%b %d, %Y") if _prev_mtg else ""
    _prev_note_str = _prev_mtg.statement_note if _prev_mtg else ""
    _prev_vote_str = f" ({_prev_mtg.vote_split})" if _prev_mtg and _prev_mtg.vote_split else ""
    _proj_note = f"Based on {_action['action'].lower()} from current {_rate_str}" if _proj_str and _rate_str else ""
    _int_vs = "Internals more hawkish" if _signal["internal_average"] > _signal["simple_average"] + 0.1 else (
        "Internals more dovish" if _signal["internal_average"] < _signal["simple_average"] - 0.1 else "Closely aligned")
    ...
    _signal_html = (
        ...
        f'<p class="signal-title-sub">Vote-weighted MPC stance with implied rate action</p>'
        f'</div><div>'
        ...
        f'<p class="signal-cell-sub">Gov 3x, DG 1.5x, Ext 1x</p></div>'
        f'<div class="signal-cell">'
    ...
    _meeting_html = (
        ...
        f'<p class="signal-meeting-val">{_prev_decision_str}{_prev_vote_str} &mdash; {_prev_date_str}</p>'
        ...
        f'<div class="signal-meeting-card">'
        f'<p class="signal-meeting-label">Internal Avg vs Committee</p>'
        f'<p class="signal-meeting-val">{_signal["internal_average"]:+.2f}'
        f' <span style="color:#64748b;font-size:0.8rem">vs</span> '
        f'{_signal["simple_average"]:+.2f}</p>'
        f'<p class="signal-meeting-note">{_int_vs}</p></div>'
        f'</div>'
    )
    ...
    if _hist_signals:
        ...
        for s in _hist_signals:
            ...
            _hist_html += (
                ...
                f'<div style="font-size:0.7rem;color:#94a3b8">{s["vote_split"]}</div>'
...
# ============================================================================
# Economic Context Panel (BOE IADB)
# ============================================================================
if boe_stats.is_available():
    st.markdown('<hr class="divider">', unsafe_allow_html=True)

    _boe = boe_stats.fetch_and_cache()
    if _boe:
        st.markdown('<p class="section-hdr">Economic Context</p>', unsafe_allow_html=True)
        st.markdown(
            '<p class="section-sub">Key UK macro indicators from BOE IADB &mdash; context for interpreting stance shifts</p>',
            unsafe_allow_html=True,
        )

        _headline_ids = ["IUDBEDR", "D7BT", "MGSX", "IHYQ"]
...
st.markdown(
    '<p class="section-sub">All MPC members ranked from most dovish to most hawkish</p>',
    unsafe_allow_html=True,
)
...
fig1.update_layout(
    **PLOTLY_LAYOUT,
    ...
    yaxis=dict(
        ...
        ticktext=[f"{r.short}  ({r.role_type})" for _, r in filtered.iterrows()],
        gridcolor=GRID,
    ),
    margin=dict(l=200, r=60, t=10, b=45),
    showlegend=False,
    bargap=0.35,
)
...
st.markdown(
    '<p class="section-sub">Each member plotted by rate stance (x) and balance sheet stance (y) '
    '&nbsp;&bull;&nbsp; Dot size indicates internal vs external</p>',
    unsafe_allow_html=True,
)
...
fig_scatter.add_trace(
    go.Scatter(
        ...
        mode="markers+text",
        marker=dict(
            size=[18 if v else 12 for v in scatter_df["is_internal"]],
...
fig_scatter.update_layout(
    **PLOTLY_LAYOUT,
    ...
    yaxis=dict(
        range=[-5.25, 5.25], gridcolor=GRID, zeroline=False,
        tickvals=[-5.0, -3.0, -1.5, 0, 1.5, 3.0, 5.0],
        title=dict(text="<- Dovish (QE)     BS Score     Hawkish (QT) ->", font=dict(size=11, color=FONT_DIM)),
    ),
...
# ============================================================================
# Chart 2 & 3 -- Composition + Internals vs Externals
# ============================================================================
st.markdown('<hr class="divider">', unsafe_allow_html=True)
...
with col_l:
    ...
    st.markdown('<p class="section-sub">Stance breakdown across all MPC members</p>', unsafe_allow_html=True)
...
with col_r:
    st.markdown(f'<p class="section-hdr">Internals vs Externals -- {stance_view}</p>', unsafe_allow_html=True)
    st.markdown(
    ...
    idf = df[df.is_internal]
    edf = df[~df.is_internal]
    ia = idf["score"].mean()
    ea = edf["score"].mean()

    fig3 = go.Figure()
    _ie_trace_names = []
    for group_df, label in [(idf, "Internal"), (edf, "External")]:
        _ie_trace_names.append(list(group_df["name"]))
        fig3.add_trace(
            go.Scatter(
                x=group_df["score"],
    ...
    for avg, label in [(ia, "Internal"), (ea, "External")]:
        fig3.add_trace(
            go.Scatter(
    ...
    ie_selection = st.plotly_chart(fig3, use_container_width=True, on_select="rerun", key="ie_click")

    _ie_pts = ie_selection.get("selection", {}).get("points", []) if ie_selection else []
    if _ie_pts:
        _ip = _ie_pts[0]
        _ip_curve = _ip.get("curve_number", 0)
        _ip_idx = _ip.get("point_number", 0)
        if _ip_curve < len(_ie_trace_names) and _ip_idx < len(_ie_trace_names[_ip_curve]):
            _ip_name = _ie_trace_names[_ip_curve][_ip_idx]
            render_evidence_panel(_ip_name, history)
...
st.markdown('<p class="section-sub">How each member\'s stance has evolved over recent months</p>', unsafe_allow_html=True)

all_names = [p.name for p in PARTICIPANTS]
defaults = [
    "Andrew Bailey", "Huw Pill", "Catherine L Mann",
    "Swati Dhingra", "Megan Greene",
]

trend_cols = st.columns([3, 1])
with trend_cols[0]:
    selected = st.multiselect("Select members", all_names, default=[n for n in defaults if n in all_names])
with trend_cols[1]:
...
if selected:
    ...
    if trend_mode == "Aggregate":
        ...
        for i, name in enumerate(selected):
            ...
            fig4.add_trace(go.Scatter(
                ...
                hovertemplate=f"<b>{name}</b> -- Policy<br>Date: %{{x}}<br>Score: %{{y:+.3f}}<extra></extra>",
            ))
            fig4.add_trace(go.Scatter(
                x=[e["date"] for e in entries],
                ...
                marker=dict(size=8, color=c, symbol="diamond",
                            line=dict(width=1, color="rgba(255,255,255,0.2)")),
                hovertemplate=f"<b>{name}</b> -- Balance Sheet<br>Date: %{{x}}<br>Score: %{{y:+.3f}}<extra></extra>",
            ))
    ...
    if _trend_dates:
        ...
        for _tm in _trend_meetings:
            _tm_label = _tm.decision.upper() if _tm.decision else "MPC"
            _tm_x = _tm.date.isoformat()
            fig4.add_vline(x=_tm_x, line_width=1, line_dash="dash", line_color="rgba(251,191,36,0.3)")
            fig4.add_annotation(
                x=_tm_x, y=4.8, text=_tm_label, showarrow=False,
                font=dict(size=8, color="rgba(251,191,36,0.5)"), yref="y",
            )
    ...
    st.info("Select members above to view trend lines.")
...
st.markdown('<p class="section-sub">Monthly stance scores across all MPC members</p>', unsafe_allow_html=True)
...
fig5.update_layout(
    **PLOTLY_LAYOUT,
    height=max(350, len(PARTICIPANTS) * 35),
...
# ============================================================================
st.markdown('<hr class="divider">', unsafe_allow_html=True)
st.markdown('<p class="section-hdr">Member Details</p>', unsafe_allow_html=True)
st.markdown('<p class="section-sub">Full roster with current stance scores across all dimensions</p>', unsafe_allow_html=True)

tbl = filtered[["name", "title", "role_type", "score", "label", "policy_score", "balance_sheet_score"]].copy()
tbl.columns = ["Name", "Title", "Role", "Score", "Stance", "Policy Score", "BS Score"]
...
st.markdown(
    '<p class="section-sub">News articles, speeches, and quotes supporting each member\'s stance classification</p>',
    unsafe_allow_html=True,
)
...
csv_current = df[["name", "title", "role_type", "overall_score", "policy_score", "balance_sheet_score"]].copy()
...
csv_current.columns = ["Name", "Title", "Role",
...
for name, entries in history.items():
    ...
    for e in entries:
        hist_rows.append(dict(
            Name=name, Role=p.role_type if p else "", Date=e["date"],
...
with dc1:
    st.download_button(
        ...
        f"boe_mpc_stances_{datetime.now():%Y-%m-%d}.csv",
        "text/csv",
    )
with dc2:
    st.download_button(
...
st.markdown(
    '<div class="foot">'
    "BOE MPC Stance Tracker &nbsp;&middot;&nbsp; "
    "Data from DuckDuckGo News, "
    '<a href="https://www.bankofengland.co.uk/news/speeches">BOE Speeches</a>, '
    '<a href="https://www.bankofengland.co.uk/monetary-policy-summary-and-minutes">MPC Minutes</a>, '
    '<a href="https://committees.parliament.uk/committee/158/treasury-committee/">Treasury Committee</a> '
    "&amp; BOE IADB"
    " &nbsp;&middot;&nbsp; Keyword-based NLP classification<br>"
    "Dual-dimension analysis: Policy (rates) + Balance Sheet (QT/QE) &nbsp;&middot;&nbsp; "
    "Vote-weighted policy signal with MPC meeting calendar<br>"
