...
Loading order
-------------
1. Import every UPPERCASE attribute from ``fomc_tracker.defaults``
2. Try ``from local.config import *`` — for each UPPERCASE name:
...
Fork users create ``local/config.py`` to customise thresholds, weights,
URLs, etc. without editing any upstream file.

This module deliberately has **zero imports from other fomc_tracker
...
from fomc_tracker import defaults as _defaults
...
try:
    from local import config as _local_cfg  # type: ignore[import-not-found]
    ...
    for _attr in dir(_local_cfg):
        ...
        if isinstance(_default_val, dict) and isinstance(_local_val, dict):
            # Merge: local keys add to / replace default keys
    ...
    logger.info("Loaded local config overrides from local/config.py")
except ImportError:
    pass  # No local/config.py — use defaults only
