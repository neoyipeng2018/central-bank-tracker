#!/usr/bin/env python3
"""CLI orchestrator: fetch news, classify stances, store history for BOE MPC."""
...
from boe_tracker.historical_data import add_stance, load_history
from boe_tracker.news_fetcher import fetch_news_for_participant, load_cached_news
from boe_tracker.participants import PARTICIPANTS, get_participant
...
def process_participant(participant, use_cache=True):
    """Fetch news, classify stance, and store for one MPC participant."""
    logger.info(f"Processing: {participant.name} ({participant.title})")
    ...
    for r in results:
        ...
        best_quote = item_evidence[0]["quote"]
        evidence.append({
            "title": r.get("title", ""),
            "url": r.get("url", ""),
    ...
    # Keep top evidence items sorted by absolute score
...
def main():
    parser = argparse.ArgumentParser(description="BOE MPC Stance Tracker - Data Fetcher")
    ...
    if args.name:
        ...
        print("\n  Fetching data for all 9 MPC members...")
        print("  " + "=" * 60)
        results = []
        for p in PARTICIPANTS:
        ...
        for p, score, label in sorted(results, key=lambda x: -x[1]):
            ...
            print(f"  {p.name:<24s} {score:+.3f}  {bar}  {label}")
        print()
